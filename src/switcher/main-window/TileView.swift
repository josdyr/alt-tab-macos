...
class TileView: FlippedView {
    ...
    private func updateAppIcon(_ element: Window, _ title: String) {
        updateDisplayedAppIcon(WebsiteIcons.enabled
            ? WebsiteIcons.icon(for: element) ?? element.icon
            : element.icon)
    }
    ...
    private func updatePositions(_ newHeight: CGFloat) {
        ...
        if Preferences.effectiveAppearanceStyle(SwitcherSession.activeShortcutIndex) != .appIcons {
            ...
            let alignTowardIcon = UserDefaults.standard.bool(forKey: "localAppNameTrailingAlignment")
            let isLeftToRight = App.shared.userInterfaceLayoutDirection == .leftToRight
            appNameLabel.alignment = isLeftToRight != alignTowardIcon ? .left : .right
